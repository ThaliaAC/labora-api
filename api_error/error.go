...
func main() {
	...
	router.HandleFunc("/itemdetails", getItemDetailss).Methods("GET")
...
func deleteItem(w http.ResponseWriter, r *http.Request) {
	...
	for index, item := range items {
		if item.ID == parameters["id"] {
			items = append(items[:index], items[index+1:]...)
			json.NewEncoder(w).Encode("The item has been remove succesfully")
			return
		}
	}
...
func getItemDetailss(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
...
func getItemDetails(id string) ItemDetails {
	...
	return ItemDetails{
		Item:    foundItem,
		Details: fmt.Sprintf("Detalles para el item %v", id),
	}
}
